...
namespace BinaryReadWrite
{
    public static class BinaryWriterExtensions
    {
        ...
        public static void WriteDictionary<T,V>(this Writer writer, Dictionary<T,V> dic)
        {
            // 'null' is encoded as '-1'
            if (dic is null)
            {
            ...
            // check if within max size, otherwise Reader can't read it.
            if (dic.Count > Reader.AllocationLimit)
                throw new IndexOutOfRangeException($"Writer.WriteDictionary - Dictionary<{typeof(T)},{typeof(V)}> too big: {dic.Count} elements. Limit: {Reader.AllocationLimit}");
            
            writer.WriteInt(dic.Count);
            foreach(KeyValuePair<T,V> kvp in dic)
            {
